			 ...
			 data-native-menu="false" id="selectCommit">
					<c:forEach items="${gitLogList}" varStatus="status" var="gitLog">
						...
						selected="selected"
							...
							value="/m/lecture/${lecture.name}/example/commit:${fn:substring(gitLog.getName(),0,8)}/filepath:">
				...
				<tbody>
					<tr>
						<td class="none-top-border td-post-writer-img" rowspan="2"><img
							src="${gitCommitLog.getImgSrc()}"></td>
						<td colspan="2" class="none-top-border post-top-title"><a rel="external"
							class="none-color"
							href="/lecture/${lecture.name}/commitlog-viewer/commit:${fn:substring(gitCommitLog.commitLogID,0,8)}">
								${fn:substring(gitCommitLog.shortMassage,0,27)}</a></td>
						<td class="none-top-border td-button" rowspan="2"><a
							rel="external"
							href="/lecture/${lecture.name}/browser/commit:${fn:substring(gitCommitLog.commitLogID,0,8)}">
