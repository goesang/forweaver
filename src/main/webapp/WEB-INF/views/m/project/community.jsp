        ...
        <div style="text-align: center;">
				<div data-theme="a" data-mini="true" data-type="horizontal"
					...
					<a id="age-desc" data-iconpos="notext" : data-theme="a"
						...
						href="/project/${project.name}/community<c:if test="${tagNames != null }">/tags:${tagNames}</c:if><c:if test="${search != null }">/search:${search}</c:if>/sort:age-desc/page:1"></a>
					<a id="age-asc" data-iconpos="notext" : data-theme="a"
						data-icon="sort-by-order-alt" data-role="button" rel="external"
						href="/project/${project.name}/community<c:if test="${tagNames != null }">/tags:${tagNames}</c:if><c:if test="${search != null }">/search:${search}</c:if>/sort:age-asc/page:1"></a>
					<a id="repost-desc" data-iconpos="notext" : data-theme="a"
						data-icon="sort-by-attributes" data-role="button" rel="external"
						href="/project/${project.name}/community<c:if test="${tagNames != null }">/tags:${tagNames}</c:if><c:if test="${search != null }">/search:${search}</c:if>/sort:repost-desc/page:1"></a>
					<a id="repost-many" data-iconpos="notext" : data-theme="a"
						data-icon="sort-by-attributes-alt" data-role="button"
						rel="external"
						href="/project/${project.name}/community<c:if test="${tagNames != null }">/tags:${tagNames}</c:if><c:if test="${search != null }">/search:${search}</c:if>/sort:repost-many/page:1"></a>
					
					<a id="repost-null" data-iconpos="notext" : data-theme="a"
						data-icon="exclamation" data-role="button" rel="external"
						href="/project/${project.name}/community<c:if test="${tagNames != null }">/tags:${tagNames}</c:if><c:if test="${search != null }">/search:${search}</c:if>/sort:repost-null/page:1"></a>
				</div>
			</div>
       ...
       <table id = "post-table" class="table table-hover">
						<c:forEach items="${posts}" var="post">
							<tr>
								...
								<td class="td-button" rowspan="2"> <c:if test="${post.kind != 2}">
