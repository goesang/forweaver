...
<body>
	<script>
		...
		var socket = new SockJS('/chat/pub/${lecture.name}');
		stompClient = Stomp.over(socket);
		stompClient.connect({}, function(frame) {
			stompClient.subscribe('/chat/sub/${lecture.name}', function(greeting) {
		...
		$(function() {
			...
			$('#btn-input').keydown(
							function(e) {
								if (e.keyCode == 13) {
									stompClient.send("/chat/pub/${lecture.name}", {"content-type": "text/plain"}, $("#btn-input").val());
									$("#btn-input").val("");
								}
							});
			...
			<div>
				<div class="panel panel-primary none-border">
					<div class="panel-heading none-border">
						<span class="glyphicon glyphicon-comment"></span> ${lecture.name}
						채팅방
					</div>
